...
`eigsh` starts from a random vector unless `v0` is pinned, so eigenvectors come back with an arbitrary overall sign that changes between runs. That is physically meaningless, $\psi$ and $-\psi$ are the same state and every observable is quadratic in $\psi$, but it makes the notebook figures irreproducible, so the solver fixes it by convention: flip $\psi$ so the point of largest $|\psi|$ comes out positive.

Taking that point with a plain `argmax` is fragile, and the test I first wrote could not have told me so. An antisymmetric state has two extrema of equal magnitude and opposite sign, so $|\psi|$ has two global maxima separated only by rounding, measured at $7\times10^{-15}$ relative for $n=1$ of the oscillator. `argmax` returns whichever came out larger, which is not guaranteed to be the same one next run. And the test read the sign *at* `argmax`, which the convention forces positive by construction, so it returned $+1$ whether or not anything had been made reproducible. It still failed under mutation, because deleting the fix deleted the thing it was tautologically reading back, and that made it look valid.
